...
```json
{
  ...
  "max_grad_evals": 90000,
  "baseline_eval_every_n_grads": 3600,
  "adaptive_eval_every_n_grads": 3600,
  "max_outer": 1000000,
  "max_inner": 25,
...
BUDGET VARIANT: identical to the K=6 sweep configuration except
max_grad_evals raised 30k -> 90k (both methods; checkpoints scale with it).
Purpose: test the budget hypothesis for the K=6 anomaly — at 30k the
baseline's curve had flattened while the adaptive method's was still
descending, so the equal-budget ratio may simply have been read too early.
...
|---|---|---|
| final best-so-far GN* | 3.551e-03 | 4.745e-03 |
| plateau found | True | False |
| plateau level | 3.551e-03 | n/a |
| plateau onset (grad evals) | 54000 | None |
| CPU time to common target 4.745e-03 | 10.17 s | 1685.10 s |
| grad evals to common target | 43200 | 75600 |

Plateau ratio (baseline / adaptive): **n/a**  
CPU-time ratio to common target (baseline / adaptive): **0.01**  
Gradient ratio to common target (baseline / adaptive): **0.57**  
