...
```json
{
  ...
  "max_grad_evals": 240000,
  "baseline_eval_every_n_grads": 8000,
  "adaptive_eval_every_n_grads": 8000,
  "max_outer": 1000000,
  "max_inner": 25,
...
Parameter dimension d = 58; data seed = 7, init seed = 8.
...
BUDGET VARIANT (stage 2): 240k gradients, 8x the sweep budget. At 30k the
equal-budget ratio was 0.25 (adaptive worse); at 90k it improved to 0.75
with the baseline plateaued at 3.55e-03 since 54k gradients and the
adaptive method still descending in intermittent steps. This run decides
whether the adaptive method crosses the baseline's floor given room.
...
|---|---|---|
| final best-so-far GN* | 3.629e-03 | 1.173e-03 |
| plateau found | True | False |
| plateau level | 3.629e-03 | n/a |
| plateau onset (grad evals) | 56028 | None |
| CPU time to common target 3.629e-03 | 15.53 s | 2519.58 s |
| grad evals to common target | 64032 | 105300 |

Plateau ratio (baseline / adaptive): **n/a**  
CPU-time ratio to common target (baseline / adaptive): **0.01**  
Gradient ratio to common target (baseline / adaptive): **0.61**  
(ratios > 1 mean the adaptive method is better)

## Health checks

- `L_scale_final` = 16.0 (>1 means the probe-estimated smoothness constants were raised at runtime by the descent safeguard; expected on ReLU MLPs)
