...
1. **Gram 化 λ-search**——精确恒等改写 GN(λ)=min_i λ^T M_i λ；本次实测
   λ-search 耗时 **428.7 s / 总 1225.0 s
   （35.0%）**，对比 7 月 11 日原版 run 的 ~95%。
...
4. **交付时修剪**——bundle 1501 -> 913 点
   （r=10 simplex 网格 + 末轮搜索赢家的 λ-激活检测）；
   探针 λ 上的 GN 值逐位不变。

结果：stop_reason=`round_fuse`，最终自报告 best-so-far GN =
**1.5258e-01**，消耗 grad-equivalents = 28169
（joint 调用 1500 次，minibatch IFO 159744000），
墙钟 1225.0 s。L_scale_final=1.0，
inner_cap_hits=150。
...
grads 达到，fast 于 182.05480551719666 s /
6760.6272 grad-equivalents 达到
（比值：CPU 13.220260331312877，
梯度 15.309230480864262）。

对原版 adaptive：共同目标 GN 0.1526，原版于
10375.001036643982 s / 22500.0
grads 达到，fast 于 1062.6116216182709 s /
25540.1472 grad-equivalents 达到。
...
论证（inner_cap_hits=150；warning 已记录）。

## 参数迭代记录（v1 -> v2）

v1（方案默认 b=1024, beta=0.9, c=0.1；不带 `_v2` 后缀的文件夹）在
best-so-far GN ~0.774 处饱和（方差地板；safeguard 静默、148/150 轮触段帽）。
本次 run（v2）按方案文档调参表执行：b 1024 -> 4096（梯度方差 /4）、
beta 0.9 -> 0.5（heavy-ball 噪声放大 10 倍 -> 2 倍）、c 不变；设计上每段
成本不变（p_seg = ceil(n/b)：49 -> 13）。结果：不再饱和——最终 best-so-far
0.1526，与原版方法 0.1473 同水平，同样 150 轮 fuse 下墙钟约省 8.5 倍
（1225 s vs 10375 s）。
