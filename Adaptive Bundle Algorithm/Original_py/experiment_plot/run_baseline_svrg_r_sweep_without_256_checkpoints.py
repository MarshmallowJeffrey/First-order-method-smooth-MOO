...
the baseline trajectory LINES are restored on both figures; later the
same day (Note §6) the lines were switched to the strict full-simplex
prefix meter (``delivered_gn_strict_history``).

EDITED July 26, 2026 (user decision, session 12; see
``Note/Jul_26_note.md`` part 2): the lines revert to the NATIVE grid
meter (max over grid nodes of the best-known value — the July-8
baseline metric, ``cov_history``): at checkpoints the baseline speaks
in its own contract's units, and the strict full-simplex score appears
ONLY as a separate delivery-time AUDIT — an x marker joined to the
grid-certificate endpoint by a dotted vertical connector.  The
vertical gap IS the measured between-node error of that grid.  The
strict prefix history stays in the summaries, no longer plotted.
Also added: ``--out-dirname`` (redirect the output folder) and
``--fast-ref`` (adaptive reference curve from another fast-trial
folder).  Engine untouched.
...
SWEEP_DIRNAME = "pure_budget_without_256_checkpoints_SVRG_IPOPT_Baseline/baseline_svrg_multi_r_vs_fast_without_256_checkpoints"
FAST_REF_CANDIDATES = [
    OUTPUT_ROOT / "fast_method_trials_v1v2v3_IPOPT" / "v3_rel_target_two_tier",
    OUTPUT_ROOT / "fast_method_trials_v1v2v3_IPOPT"
...
def _load_fast_curves(ref_override=None):
    cands = (([Path(ref_override)] if ref_override else [])
             + FAST_REF_CANDIDATES)
    for cand in cands:
        p = cand / "summary.json"
        if p.exists():
...
def _plot_sweep(results: dict, fast: dict, fast_path: str, args,
    ...
    for axis_key, x_label, fname, floor_per_node, x_log in (
        ...
        for j, r in enumerate(rs):
            ...
            # Trajectory line, NATIVE grid meter (Jul 26, user decision;
            # replaces the Jul 25 §6 comparable-meter lines): max over
            # ALL grid nodes of the best-known value at each checkpoint
            # (x0-initialised lag semantics — the July-8 baseline
            # metric).  The strict full-simplex prefix history stays in
            # the summaries (delivered_gn_strict_history), not plotted.
            hist_y = res.get("cov_history")
            if hist_y:
            ...
            grid_end = (float(hist_y[-1]) if hist_y
                        else float(res.get("grid_worst_best_val", np.nan)))
            audit = float(res["delivered_gn_strict"])
            # Endpoint = the run's own grid certificate (small circle);
            # the strict full-simplex score is a SEPARATE delivery-time
            # AUDIT (x marker), joined by a dotted vertical connector at
            # the same abscissa: the vertical gap IS the measured
            # between-node error of that grid.
            ax.scatter([x_end], [grid_end], marker="o", s=30, zorder=5,
                       facecolors=(color if filled else "none"),
                       edgecolor=("black" if filled else color),
                       linewidth=(0.7 if filled else 1.3))
            ax.plot([x_end, x_end], [grid_end, audit], color=color,
                    ls=":", lw=1.1, alpha=0.9, zorder=4)
            ax.scatter([x_end], [audit], marker="x", s=48, zorder=6,
        ...
        proxy_c = reds(0.7)
        ax.plot([], [], color=proxy_c, lw=1.5,
                label="Baseline-SVRG grid meter (nodes only)")
        ax.scatter([], [], marker="o", s=30, facecolors=proxy_c,
                   edgecolor="black", linewidth=0.7,
                   label="grid certificate end")
        ax.scatter([], [], marker="x", s=48, color=proxy_c, linewidth=1.6,
                   label="simplex audit (strict, full simplex)")
        ...
        ax.set_ylabel("GN meter (lines: native meters; x: full-simplex audit)")
        ax.set_title(
            f"MLP K={K}, p={cfg['p']}, n={cfg['n']}, "
            f"h={cfg['hidden_sizes']}, {cfg['activation']} — "
            f"Baseline-SVRG r-sweep (grid meter + end audit) vs fast adaptive\n"
...
每个 r 画成一条轨迹线，用 baseline 自己的网格口径（7 月 26 日用户
决定，撤销 7 月 25 日 §6 的可比口径画法）：每个 checkpoint 时刻所有
网格节点最好已知值的最大值（x0 初始化的滞后语义，即 7 月 8 日
baseline 口径，`cov_history`）。线终点的小圆圈 = 该 run 自己的网格
证书终值。交付点集的严格档 64 起点全 simplex GN* 只作为单独的交付时
AUDIT 出现：x 记号，与圆圈之间用竖虚线相连。这段竖直落差就是该网格
被实测出的节点间误差——baseline 自己的口径看不见的那部分。口径提醒：
baseline 线（只看网格节点）与 fast 线（其自身粗档 λ-search）是不同
口径；只有 x audit 是全 simplex 口径，跨曲线对读在 audit 记号处才
严格成立，其余位置仅作参考。严格前缀轨迹保留在各 summary 的
`delivered_gn_strict_history`，不再入图。空心圆圈（如有）= 触保险丝/
censored 的 run：该点是此容差下该 r 成本的下界，不是收敛测量。
...
def main() -> None:
    ...
    parser.add_argument("--out-dirname", type=str, default=None,
                        help="output folder under output/ (may contain "
                             "subdirs); default: the canonical sweep folder")
    parser.add_argument("--fast-ref", type=str, default=None,
                        help="fast-trial folder whose summary.json supplies "
                             "the adaptive reference curve; default: v3")
    ...
    dirname = (args.out_dirname if args.out_dirname
               else SWEEP_DIRNAME + ("_SMOKE" if args.smoke else ""))
    ...
    fast, fast_path = (None, "")
    if not args.smoke:
        fast, fast_path = _load_fast_curves(args.fast_ref)
