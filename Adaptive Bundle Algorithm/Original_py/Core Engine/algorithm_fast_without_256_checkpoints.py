...
def _bundle_update_msvrg(
    ...
    *,
    ...
    on_full_eval: Optional[Callable[[int], None]] = None,
) -> Dict:
    ...
    on_full_eval (Jul 27, recording only): called after EVERY segment-end
    full evaluation joins the bundle, with the running full_evals count
    of this call.  Used by the caller for per-segment checkpoint
    recording; it must not mutate solver state.  Default None = no-op.
    """
    ...
    for _seg in range(max_segments):
        ...
        while True:
            ...
            full_evals += 1
            _append_cache_row()
            if on_full_eval is not None:
                on_full_eval(full_evals)
            F_y = float(Fl[mcur - 1])
...
def algorithm_adaptive_fast(
    ...
    *,
    ...
    # Jul 27 (recording only, default off = bit-identical histories):
    # when True, a checkpoint is also recorded after every segment-end
    # full evaluation inside the inner loop, subject to the same
    # eval_every_n_grads cadence (None = every segment).  No solver
    # decision reads these records.
    ...
    def _seg_ckpt(fe_so_far: int) -> None:
        # Jul 27, recording only: refresh the running grad counter and
        # record a checkpoint when the cadence is due.  grad_equiv is
        # recomputed from scratch after the inner loop returns, so this
        # assignment cannot drift; no solver decision reads it mid-inner.
