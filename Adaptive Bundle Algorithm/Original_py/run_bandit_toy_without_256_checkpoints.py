...
Figures (5): GN* vs CPU, GN* vs grads, Pareto front, value-gap per-w
profile, value-gap convergence (2 panels).
...
def fig_gn_curves(bl: Dict, ad_strict: List[float], ad: Dict, eps: float,
    ...
    for xkey, xlabel, fname, logx in (
        ("cpu_times", "CPU time (s), method work only", "fig1_gn_vs_cpu.png",
         True),
        ...
        if logx:
            ...
            ax.set_xscale("log")
        ax.plot(x_bl, np.minimum.accumulate(bl["delivered_gn_strict_history"]),
                marker="s", ms=3.5, color=BASELINE_COLOR,
                label=baseline_label)
        ax.plot(x_ad, np.minimum.accumulate(ad_strict), marker="o", ms=3.5,
        ...
        budget = min(x_bl[-1], x_ad[-1])
        ax.axvline(budget, color="k", ls="-.", lw=1, alpha=0.6,
                   label="equal-budget marker")
        ax.set_yscale("log")
...
def _solution_map_front(fvals: np.ndarray, ws: np.ndarray) -> np.ndarray:
    """Answer each query w with the delivered point minimising F_w."""
    scal = fvals @ np.stack([ws, 1.0 - ws])
    return fvals[np.argmin(scal, axis=0)]


def fig_pareto(problem, cfg: Dict, bl_fvals: np.ndarray, ad_fvals: np.ndarray,
               bl_cloud: np.ndarray, ad_cloud: np.ndarray,
    ...
    # Jul 26 fix: the headline markers are each method's answers to the
    # SAME 12 query weights (uniform), so the two fronts have identical
    # counts; the raw evaluated-point clouds stay as faint background.
    query_ws = np.linspace(0.0, 1.0, cfg["n_pf_ref_points"])
    bl_front = _solution_map_front(bl_fvals, query_ws)
    ad_front = _solution_map_front(ad_fvals, query_ws)
    n_bl_distinct = np.unique(bl_front, axis=0).shape[0]
    n_ad_distinct = np.unique(ad_front, axis=0).shape[0]
    ...
    ax.plot(ref_pts[:, 0], ref_pts[:, 1], "*", color="0.6", ms=10, mew=0,
            label="SURF arc-uniform reference (12 pts)")
    ...
    ax.plot(bl_front[:, 0], bl_front[:, 1], "s", color=BASELINE_COLOR, ms=7,
            mec="white", mew=0.5,
            label=f"baseline front @ 12 queries ({n_bl_distinct} distinct)")
    ax.plot(ad_front[:, 0], ad_front[:, 1], "o", color=ADAPTIVE_COLOR, ms=7,
            mec="white", mew=0.5,
            label=f"adaptive front @ 12 queries ({n_ad_distinct} distinct)")
    ax.set_xlabel("F1(theta)")
    ax.set_ylabel("F2(theta)")
    ax.set_title(f"Offline-bandit toy Pareto front (eps = {cfg['epsilon']:g})")
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=8, loc="upper right")
    note = (
        f"baseline:  max point-to-oracle dist = "
        f"{bl_pf['max_point_to_oracle']:.3e},  IGD = {bl_pf['igd']:.3e}\n"
        f"adaptive:  max point-to-oracle dist = "
        f"{ad_pf['max_point_to_oracle']:.3e},  IGD = {ad_pf['igd']:.3e}"
    )
...
def main() -> None:
    ...
    ap.add_argument("--out-root", type=str, default=os.path.join(
        ...
        "bandit_toy_surf_without_256_checkpoints"))
    args = ap.parse_args()
    ...
    fig_pareto(problem, cfg, bl_fvals, ad_fvals_delivered,
               bl_fvals, ad_fvals_full, bl_pf, ad_pf, out_dir)
