...
def _sample_planted_data(
    ...
    peaked.  Class counts will be close to, but not exactly, n/K.  Because
    every class defines one optimisation objective, datasets containing an
    empty class are rejected and resampled.
    """
    if K <= 0:
    ...
    if n < K:
        raise ValueError(
            f"n must be at least K so every class can contain a sample; got n={n}, K={K}."
        )
    ...
    for _ in range(max_attempts):
        ...
        cumprob[:, -1] = 1.0                        # guard against round-off
    ...
    raise RuntimeError(
        f"Could not sample a dataset containing all {K} classes after "
        f"{max_attempts} attempts; increase n or reduce K."
    )


# ====================================================================
# Regularised multi-class logistic regression (strongly convex)
# NOTE: This factory is not called by the repository's current experiment
# pipeline (experiments.py uses make_mlp_nonconvex).  It is retained as a
# separate strongly-convex test problem for future experiments/comparisons.
# ====================================================================
...
def make_mlp_nonconvex(
    ...
    joint_oracle    : callable returning all objective values and gradients as
                      arrays with shapes (K,) and (K, d), respectively.
    ...
    >>> objs, grads, L, joint = make_mlp_nonconvex(K=3, p=4, n=60, h=8)
    >>> d = 8*4 + 8 + 3*8 + 3  # = 67
    >>> theta = np.zeros(d)
