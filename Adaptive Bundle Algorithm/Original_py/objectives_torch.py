...
def _sample_planted_data(
    ...
    if n < K:
        raise ValueError(
            f"n must be at least K so every class can contain a sample; "
            f"got n={n}, K={K}."
        )
...
def make_mlp_nonconvex(
    ...
    n_probes     : random parameter-pair probes used to estimate each L_i.
    ...
    if (
        not isinstance(n_probes, (int, np.integer))
        or isinstance(n_probes, (bool, np.bool_))
        or n_probes < 1
    ):
        raise ValueError(f"n_probes must be a positive integer; got {n_probes!r}.")
