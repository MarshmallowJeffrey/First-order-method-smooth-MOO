...
OUTPUT_ROOT_ORIG = HERE.parent / "output"
...
def main() -> None:
    ...
    root_name = "without_256_checkpoints"
    if args.smoke:
        root_name += "_smoke"
    ...
    plateau_summary = run_config(
        plateau_cfg, PLATEAU_DETECT, root / "plateau" / PLATEAU_TAG,
        "plateau comparison")
    crossover_summary = run_config(
        crossover_cfg, CROSSOVER_DETECT, root / "crossover" / CROSSOVER_TAG,
        "plateau comparison")

    # A/B figures against the existing with-256 runs.
    mismatches: List[str] = []
    for old_path, new_summary, cfg, stem, nice in (
        (OUTPUT_ROOT_ORIG / "plateau" / PLATEAU_TAG / "summary.json",
         plateau_summary, plateau_cfg, "ab_plateau_K5",
         "plateau K=5 (30k budget)"),
        (OUTPUT_ROOT_ORIG / "crossover" / CROSSOVER_TAG / "summary.json",
