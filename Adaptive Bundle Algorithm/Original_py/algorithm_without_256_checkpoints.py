"""
algorithm_without_256_checkpoints.py  –  Adaptive Algorithm (Algorithm 2),
A/B measurement variant.

Identical to ``algorithm.py`` in every algorithmic respect (same steps, same
lambda searches, same trajectory for the same seeds) EXCEPT what checkpoints
record: instead of running the external fixed 256-start ``pc_star`` yardstick
at every checkpoint, ``cov_history`` records the method's OWN most recent
lambda-search value (the GN estimate the algorithm itself computed with
``lambda_max_starts`` starts at the beginning of the current outer round).
Checkpoint 0 is backfilled with the round-1 search value, which scores the
identical one-point bundle {x0}.  The original file and its outputs are
untouched; see output/without_256_checkpoints/README.md for the experiment
this variant serves.
"""
...
def algorithm_adaptive(
    ...
    *,
    ...
    # Default True (changed July 8, in step with algorithm.py): a missing
    # IPOPT install now fails loudly instead of silently degrading the
    # lambda-search to SLSQP.  An explicit lambda_solver="slsqp" run must
    # also pass require_ipopt=False to acknowledge the choice.
    ...
    """Run the adaptive bundle method with SELF-REPORTED checkpoints.

    A/B measurement variant of ``algorithm.algorithm_adaptive``: identical
    trajectory, but ``cov_history`` records the method's own most recent
    lambda-search GN value (``lambda_max_starts`` starts) instead of the
    external fixed 256-start ``pc_star`` yardstick.  The self-reported
    criterion is still

        GN(B) = max_lambda min_i ||grad F_lambda(x_i)||^2,

    but maximised by the algorithm's own working search, and read at the
    start of the current outer round (it lags the bundle by that round's
    inner steps).  CPU time spent recording checkpoints is excluded from the
    ...
    def _checkpoint(label: str) -> None:
        ...
        ck_t0 = time.time()
        # A/B variant: no external 256-start pc_star solve.  Record the
        # method's own most recent lambda-search value — the GN estimate the
        # algorithm itself computed (with ``lambda_max_starts`` starts) on
        # the bundle at the START of the current outer round.  It therefore
        # lags the bundle by the current round's inner steps.  Checkpoint 0
        # has no search value yet; it is backfilled after the run with the
        # round-1 value, which scores the identical {x0} bundle.
        ...
        if verbose:
            print(
                ...
                f"| grad_evals={total_iters * K} | self-reported pc={cov:.4e}"
            )
    ...
    # Backfill checkpoint 0 (recorded before any lambda search had run) with
    # the round-1 search value: both score the same one-point bundle {x0}.
