...
def baseline_svrg_certified(
    ...
    *,
    ...
    # delivery, subject to the same eval_every_n_grads cadence (None =
    # every segment).  The node-level checkpoint block — and with it the
    # equal-level stop check cadence — is unchanged.
    ...
    for idx in range(N):
        ...
        for _seg in range(msvrg_max_segments):
            ...
            while True:
                ...
                # Jul 27, recording only (default off): per-segment
                # checkpoint under the same cadence.  The node-level
                # block below still records/stop-checks exactly as
                # before.
                if record_segment_checkpoints and (
                        eval_every_n_grads is None
                        or (grad_equiv - grad_at_ckpt)
                        >= eval_every_n_grads):
                    _checkpoint(f"node {idx + 1} seg")
