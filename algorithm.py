...
def _worst_case_subopt_fast(bundle: Bundle, reference_map: Dict,
                            objectives: List[Callable], K: int,
                            grad_objectives: Optional[List[Callable]] = None,
                            metric: str = "gap") -> float:
    """Fast worst-case-over-the-fine-grid quality metric for the A2/A4 map.

    Reconstructs the solution map  x̂(λ) = T_map(B, λ)  at every fine-grid
    λ (batched over the grid in a single einsum), then aggregates one of
    two worst-case quality measures depending on ``metric``:

    * ``metric="gap"`` (default):  function-value suboptimality
          err = sup_λ [ F_λ(x̂(λ)) − F*_λ ].
      Appropriate for (strongly) convex problems where ``F*_λ`` from the
      reference map is a meaningful global optimum (e.g. logreg/GAP).

    * ``metric="gradnorm"``:  worst-case squared gradient norm
          err = sup_λ ‖∇F_λ(x̂(λ))‖²,   ∇F_λ(x) = Σ_k λ_k ∇F_k(x).
      A first-order *stationarity* measure that does NOT reference ``F*``.
      Appropriate for non-convex problems (e.g. the MLP), where the
      function-value gap to a non-global reference is misleading.  Requires
      ``grad_objectives``.

    The per-grid-point objective/gradient evaluations remain Python-level
    (the closures are over per-class sample indices), but those are the
    only Python-level work in the checkpoint.
    """
    ...
    if metric == "gradnorm":
        ...
        for n, lam in enumerate(fine_grid):
            x_n = X_hat[n]
            # ∇F_λ(x_n) = Σ_k λ_k ∇F_k(x_n)
            g = np.zeros_like(x_n)
            for k in range(K):
                g = g + lam[k] * grad_objectives[k](x_n)
            gn = float(np.dot(g, g))
            if gn > worst:
                worst = gn
        return worst

    # default: function-value gap to the reference map
...
def algorithm2_progressive(
    ...
    * ``epsilon=ε > 0`` (Algorithm 2 from the paper, App. B.1):
    ...
    err0 = -np.inf
    if metric == "gradnorm":
        # initial map is the constant point x0:  err0 = sup_λ ‖∇F_λ(x0)‖²
        g_each = [grad_objectives[k](x0) for k in range(K)]   # (K, d)
